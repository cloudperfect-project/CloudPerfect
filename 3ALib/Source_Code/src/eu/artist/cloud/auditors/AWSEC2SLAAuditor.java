...
public class AWSEC2SLAAuditor implements AuditingInterface {
	...
	public  void checkApplicabilityConditions(String provider,String user,String apiKey, String databaseIP, String DBuser, String DBkey,String libUser) throws Exception {
		...
		try {
			...
			//log pure Provider API response times
			...
			try {
			    PrintWriter out = new PrintWriter(new BufferedWriter(new FileWriter("pureAmazonAPIcalls.txt", true)));
			...
			//	addReachabilityInfo(false,metadata); 
				...
				if((metadata.getStatus().toString()).equals("RUNNING")){  //(metadata.getStatus().toString()).equals("RUNNING")
					isRunning=true;
					System.out.println("YES");
					NodeList.remove(0);		//--------------------->edw to theloume auto??	
					for (ComputeMetadata node2 : NodeList){
						...
						if ((metadata.getLocation().getParent().getId()).equals(metadata2.getLocation().getParent().getId())){
							...
							if (((metadata.getStatus().toString()).equals("RUNNING"))&& ((metadata2.getStatus().toString()).equals("RUNNING"))){//SUSPENDED->RUNNING
...
/*
			...
			for (String validTemplate: validTemplateIDs){
				...
				loggedData.add(addReachabilityInfo(reachabilityPerTemplateID,thisinfo,libUser,provider));//Maybe Qoeuser
			}
			...
			//FIX--------> AN EXW 3 AZ TOTE PITHANON KAI TA 2 EINAI UNREACHABLE KAI TO 1 REACHABLE TOTE THA VGALEI REACHABLE
			//SOS-------->FIX THIS
	...
	public String addReachabilityInfo(boolean isReachable,NodeMetadata thismetadata,String libuser,String provider ){
		String loggedRecord;
		...
		if (isReachable){
			...
			jo.addProperty("User-Id", libuser);
			jo.addProperty("Provider", provider); //User-provider fields user may change to Qoe user.
