...
public class AbstractedAvailabilityLogger {
	 ...
	 */
	public void logAvailability(ArrayList<String> TemplateIDs,String DBuser,String DBpass, String databaseIP) throws UnknownHostException, MongoException, FileNotFoundException {
		...
		Mongo mongoClient = new Mongo( "147.102.19.75" , 80 );
...
/*		
		...
		FileInputStream input = new FileInputStream("3alibconfig.properties");
		...
		if (overall.contains("datastore")){
			coll = db.getCollection("datastore");
		}else if (overall.equals("microsoft-azureblob")){
				
			coll = db.getCollection("azureblob");
		}
		else {
			coll = db.getCollection("log_samples");
		}
	*/	
