...
public class AvailabilityAuditor extends Thread {
	...
	public synchronized String getLibuser() {
		return libuser;
	}



	public synchronized void setLibuser(String libuser) {
	...
	public void run() {
		...
		if (overall.equals("Cosmote-Compute")) {
			OpenstackCosmoteAuditor kappa=new OpenstackCosmoteAuditor();
			try {
				kappa.checkApplicabilityConditions(overall,user,apikey, "a","a", "a",libuser);
			} catch (Exception e) {
		...
		if (overall.equals("ULM-Compute")) {
			OpenstackUlmAuditor kappa=new OpenstackUlmAuditor();
			try {
				kappa.checkApplicabilityConditions(overall,user,apikey, "a","a", "a",libuser);
			} catch (Exception e) {
				// TODO Auto-generated catch block
		...
		if (overall.equals("aws-ec2")){
			...
			newaud.checkApplicabilityConditions(overall,user,apikey, dbhost,dbuser, dbpass,libuser);
		...
		if (overall.equals("Filab-Compute")) {
			OpenstackFilabAuditor kappa=new OpenstackFilabAuditor();
			try {
				kappa.checkApplicabilityConditions(overall,user,apikey, "a","a", "a",libuser);
			} catch (Exception e) {
				// TODO Auto-generated catch block
		...
		if (overall.equals("microsoft-azureblob")){//check if naming has effect-has dependency on jclouds
			...
			AzureBlobLogParser newparser=new AzureBlobLogParser();
			
			try {
				newparser.parseLogs(path,libuser,dbhost,dbuser, dbpass);
			} catch (Exception e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
