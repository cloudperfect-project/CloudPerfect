...
public class PidstatHandler implements MonitoringCommandHandler{
	...
	public synchronized void commandStopper(){
		try {
			
			UnixProcessUtils.killUnixProcess(commandExecutor.getProcess(), info.pass, "pidstat");
			...
			try {
				...
				commandExecutor.getProcess().destroy();
				
			}
			commandExecutor.join();
	//		System.out.println("+++DEBUG: FINISHED WAITING");
			
			StringBuilder stdout = commandExecutor.getStandardOutputFromCommand();
			
			//the following command gets from the output ONLY the Average values!!
